...
namespace TeamPulse.Integration
{
    public class AuthenticationHelper
    {
        ...
        private readonly bool authenticateWithUserAndPassword;
        ...
        private string LoginUrl
        {
            get
            {
                return TeamPulseUrl + "/Authentication/RichAppLogin";
            }
        }
        ...
        public AuthenticationHelper(string teamPulseUrl, string refreshToken, bool authenticateWithUserAndPassword, string username, string password)
        {
            this.RefreshToken = refreshToken;
            this.password = password;
            this.username = username;
            this.authenticateWithUserAndPassword = authenticateWithUserAndPassword;
                ...
                case HttpStatusCode.Unauthorized:
                    {
                        ...
                        {
                            if (authenticateWithUserAndPassword)
                            {
                                string refreshToken;
                            ...
                            {
                                string refreshToken;
                                AccessToken = AcquireAccessTokenWithRichApp(location, clientId, LoginUrl, out refreshToken);
                                RefreshToken = refreshToken;
                            }
                        }
                        return AccessToken;
        ...
        private static string AcquireAccessTokenWithPassword(string location, string clientId, string username, string password, out string refreshToken)
        {
            ...
            switch (responseCode)
            {
                ...
                default:
                    throw new ApplicationException("The server returned response code {0}." + responseCode);
            }
        }

        #region AcquireAccessTokenWithRichApp
         
        private static string AcquireAccessTokenWithRichApp(string location, string clientId, string loginUrl, out string refreshToken)
        {
            refreshToken = null;
            string accessToken = null;
            ...
            ie.TitleChange += new SHDocVw.DWebBrowserEvents2_TitleChangeEventHandler(IE_TitleChange);
            ie.Navigate(loginUrl);
             
            //Waiting response form Internet Explorer...
            while (true)
            {
                Thread.Sleep(100);
                ...
                if (quit)
                {
                    throw new ApplicationException("Authentication was canceled.");
                }
                ...
                if (!String.IsNullOrEmpty(browserTitle))
                {
                    ...
                    var code = ReadVerificationCodeFromTitle(browserTitle);

                    if (!String.IsNullOrEmpty(code))
                    {
                        accessToken = GetTheAccessCodeForVerificationCode(location, clientId, code, ref refreshToken);
                    }
                    break;
                }
            }
        ...
        static bool quit;
        static string browserTitle;
         
        static void IE_TitleChange(string Text)
        {
            if (Text.StartsWith("Authentication Delegation,"))
                browserTitle = Text;
        }
         
        static void IE_OnQuit()
        {
            quit = true;
        }
